...
class app(tk.Tk):
            ...
            if self.cur_sel_page != in_str:    
                ...
                if in_str == "setting":
                    ...
                    if in_str == "generate_num":
                        ...
                        self.gene_multiple_gene_state_0["text"] = "未输入任何 *.csv 文件。"
    ...
    def generate_page(self):
        ...
        gene_single_en_name_frame_line_1_right.grid(row = 0, column = 1,
                                                    padx = 2, 
                                                    sticky = tk.NW)
        temp_str_0 = "(1) 合格的格式要求是或者"
        temp_str_0 = temp_str_0+"\"名字的全部3个部分都是空白\"作为\"没有提供名字\", "
        temp_str_0 = temp_str_0+"或者\"至少名是非空白的\"作为\"提供了名字\". "
        ttk.Label(gene_single_en_name_frame_line_1_right, 
                  text = temp_str_0,
                  wraplength = 1000, 
                  font = self.fonsize_s).grid(row = 0, column = 0,
                                              sticky = tk.W)
        temp_str_0 = "(2) 作为英文名字的任何一个部分，如果它不是空白的，"
        temp_str_0 = temp_str_0+"它的初始字符必须来自26个英文大写字母，"
        temp_str_0 = temp_str_0+"并且其余的字符必须来自26个英文大写字母、"
        temp_str_0 = temp_str_0+"26个英文小写字母、空格\" \"、横线\"-\"、单引号\"'\"和\"‘\"和\"’\"。"
        ttk.Label(gene_single_en_name_frame_line_1_right, 
                  text = temp_str_0,
                  wraplength = 1000, 
        ...
        gene_single_vn_name_frame_line_1_right.grid(row = 0, column = 1,
                                                    padx = 2, 
                                                    sticky = tk.NW)
        temp_str_0 = "(1) 其他名字 / 虚拟名字 会被视为空白，"
        temp_str_0 = temp_str_0+"如果类型是\"None\"或者名字为空白。"
        ttk.Label(gene_single_vn_name_frame_line_1_right, 
                  text = temp_str_0,
                  wraplength = 1000, 
                  font = self.fonsize_s).grid(row = 0, column = 0,
                                              sticky = tk.W)
        temp_str_0 = "(2) 类型中，\"None\"是\"空白\"，"
        temp_str_0 = temp_str_0+"\"Name in other language\"是\"其它语言的名字\"，"
        temp_str_0 = temp_str_0+"\"Social media APP\"是\"社交媒体应用程序\"，"
        temp_str_0 = temp_str_0+"\"Other\"是\"其他\"。"
        ttk.Label(gene_single_vn_name_frame_line_1_right, 
                  text = temp_str_0,
                  wraplength = 1000, 
                  font = self.fonsize_s).grid(row = 1, column = 0,
                                              sticky = tk.W)
        temp_str_0 = "(3) 在名字和附加(@或#)里的字符的范围是Unicode的U+0020至U+FFFF, "
        temp_str_0 = temp_str_0+"而在它们中出现了引号\"\"\"和\"'\"则会被取代为\"?\"。"
        ttk.Label(gene_single_vn_name_frame_line_1_right, 
                  text = temp_str_0,
                  wraplength = 1000, 
                  font = self.fonsize_s).grid(row = 2, column = 0,
                                              sticky = tk.W)
        temp_str_0 = "(4) 至少，英文名字 或 其他名字 / 虚拟名字 中的一个是非空白的。"
        ttk.Label(gene_single_vn_name_frame_line_1_right, 
                  text = temp_str_0,
                  wraplength = 1000, 
        ...
        self.cur_gene_import_csv_str_list = None
        self.cur_gene_import_csv_title = None
        temp_str = "(1) 在*.csv文件的第一行，标题行，必须有6个或7个格子（无论大写还是小写字母）: "
        temp_str = temp_str+"(i) \"Given Names\"或\"GN\", "
        temp_str = temp_str+"(ii) \"Middle Names\"或\"MN\", "
        temp_str = temp_str+"(iii) \"Family Names\"或\"FN\", "
        temp_str = temp_str+"(iv) \"Another Types\"或\"Virtual Types\"或\"AT\"或\"VT\", "
        temp_str = temp_str+"(v) \"Another Names\"或\"Virtual Names\"或\"AN\"或\"VN\", "
        temp_str = temp_str+"(vi) \"Another Additions\"或\"Virtual Additions\"或\"AA\"或\"VA\", "
        temp_str = temp_str+"(vii) 一个可选的格子\"Index\"或\"No\"."
        ttk.Label(generate_page_multiple_import_frame_line_1, 
                  text = temp_str,
                  wraplength = 1100, 
                  font = self.fonsize_s).grid(row = 0, column = 0,
                                              sticky = tk.NW) 
        temp_str = "(2) 最大的行数（除去第一行，标题行）是20 000。"
        ttk.Label(generate_page_multiple_import_frame_line_1, 
                  text = temp_str,
                  wraplength = 1100, 
                  font = self.fonsize_s).grid(row = 1, column = 0,
    ...
    def generate_fun_single_gene(self):
        ...
        if not temp_bool_0:
            ...
            if not temp_bool_1:
                showerror(title = "错误", 
                          message = "存在其它的*.iden或*.txt、*.csv在'"+self.basic_parameter[1][0]+"/', 或子文件夹中。")
            else:
                        ...
                        if not temp_list_2 is None:
                            ...
                            if not temp_list_3 is None:
                                ...
                                with open(temp_str_1_0, "w", encoding = "utf-8") as save_file:
                                    ...
                                    save_file.close()
                                temp_str_2 = "文件'"+temp_str_1_0+"'已输出。"
                                if self.basic_parameter[1][2][1]:
                                    ...
                                    temp_str_2 = temp_str_2+"文件'"+temp_str_1_0+"'已输出。"
                                    with open(temp_str_1_0, "w", encoding = "utf-8") as save_file:
                                ...
                                if self.basic_parameter[1][2][3]:
                                    ...
                                    with open(temp_str_1_0, "w", encoding = "utf-8") as save_file:
                                        ...
                                        save_file.close()
                                showinfo(title = "输出*.iden (和*txt)文件", 
                                         message = temp_str_2) 
                                self.gene_single_gn_entry.delete(0, "end")
                                self.gene_single_mn_entry.delete(0, "end")
                                self.gene_single_fn_entry.delete(0, "end")
                        ...
                        if (valid_gn_bool & valid_mn_bool & valid_fn_bool & 
                            ...
                            if temp_len_1 > 0:
                                ...
                                if (temp_len_1 > 0) & (temp_len_1 <= 20000):
                                    ...
                                    showerror(title = "错误", 
                                              message = "行数（除了第一行，标题行）的范围必须在1-20 000。") 
                            else:
                                showerror(title = "错误", 
                                          message = "行数（除了第一行，标题行）的范围必须在1-20 000。") 
                        else:
                            showerror(title = "错误", 
                                      message = "第一行，标题行 无法读取") 
                    else:
                        showerror(title = "错误", 
                                  message = "第一行，标题行 无法读取") 
    ...
    def generate_fun_multiple_clean(self):
        if not self.cur_progress_bool:
            if ((not self.cur_gene_import_csv_str_list is None) | 
                ...
                answer = askyesno(title="清除",
                                  message="您确定要清除已输入的*.csv文件?")
                if answer:  
                    self.cur_gene_import_csv_str_list = None
                    self.cur_gene_import_csv_title = None
                    self.gene_multiple_gene_state_0["text"] = "未输入任何 *.csv 文件。"
                        ...
                        if not temp_bool_1:
                            ...
                            for n in range(row_num):
                                if self.cur_progress_bool:
                                    ...
                                    break
                            if self.cur_progress_bool:
                                self.gene_multiple_gene_state_0_0["text"] = "合格: "+str(temp_success_num)
                                self.gene_multiple_gene_state_0_1["text"] = "不合格: "+str(temp_failure_num)
                            ...
                            if self.cur_progress_bool:
                                ...
                                temp_str_0 = temp_str_0+'"'+"索引号"+'"'+","
                                temp_str_0 = temp_str_0+'"'+"合格"+'"'+","
                                temp_str_0 = temp_str_0+'"'+"身份文件"+'"'+","
                                temp_str_0 = temp_str_0+'"'+"混合编号 (mixed number)"+'"'+","
                                temp_str_0 = temp_str_0+'"'+"成员编号 (member number)"+'"'+","
                                temp_str_0 = temp_str_0+'"'+"名"+'"'+","
                                temp_str_0 = temp_str_0+'"'+"中间名"+'"'+","
                                temp_str_0 = temp_str_0+'"'+"姓"+'"'+","
                                temp_str_0 = temp_str_0+'"'+"其他名字 / 虚拟名字 的类型"+'"'+","
                                temp_str_0 = temp_str_0+'"'+"其他名字 / 虚拟名字"+'"'+","
                                temp_str_0 = temp_str_0+'"'+"附加 (@或#)"+'"'+","
                                temp_str_0 = temp_str_0+'"'+"日期"+'"'+","
                                temp_str_0 = temp_str_0+'"'+"时间"+'"'
                                ...
                                if self.cur_progress_bool:
                                    ...
                                    self.cur_progress_bool = False
                                    self.cur_multi_gene_bool = False
                                    self.gene_multiple_gene_state_0["text"] = "完成。"                                
                                    self.gene_multiple_gene_state_2["text"] = "'"+temp_path_0+"'已输出。"
    
    def generate_fun_multiple_stop(self):
        if self.cur_progress_bool:
            answer = askyesno(title="停止当前过程",
                              message="您确定要停止当前的过程么?")
            if answer:  
                self.cur_progress_bool = False
                self.cur_multi_gene_bool = False
                self.gene_multiple_gene_state_0["text"] = "已停止。"
    ...
    def check_validity_page(self):
        ...
        check_valid_page_mix_num_frame_multiple_line_1.grid(row = 1, column = 0,
                                                            padx = 2, pady = 2,
                                                            sticky = tk.W)
        temp_str = "(1) 在*.csv文件的第一行，标题行中，有一个必须的格子\"Mixed Numbers\"或\"Mix\"，以及7个可选的格子(无论大写还是小写字母): "
        temp_str = temp_str+"(i) \"Given Names\"或\"GN\", "
        temp_str = temp_str+"(ii) \"Family Names\"或\"FN\", "
        temp_str = temp_str+"(iii) \"Issued Year\"或\"Iss Y\", "
        temp_str = temp_str+"(iv) \"Issued Month\"或\"Iss M\", "
        temp_str = temp_str+"(v) \"Issued Day\"或\"Iss D\", "
        temp_str = temp_str+"(vi) \"Issued Hour\"或\"Iss H\", "
        temp_str = temp_str+"(vii) \"Issued Minute\"或\"Iss Min\"."
        ttk.Label(check_valid_page_mix_num_frame_multiple_line_1, 
                  text = temp_str,
                  wraplength = 1100, 
                  font = self.fonsize_s).grid(row = 0, column = 0,
                                              sticky = tk.NW) 
        temp_str = "(2) 最大行数(除了第一行，标题行)是1 000."
        ttk.Label(check_valid_page_mix_num_frame_multiple_line_1, 
                  text = temp_str,
                  wraplength = 1100, 
                  font = self.fonsize_s).grid(row = 1, column = 0,
        ...
        check_valid_page_mem_num_frame_multiple_line_1.grid(row = 1, column = 0,
                                                            padx = 2, pady = 2,
                                                            sticky = tk.W)
        temp_str = "(1) 在*.csv文件的第一行，标题行中，有一个必须的格子\"Member Numbers\"或\"Mem\"，以及3个可选的格子(无论大写还是小写字母): "
        temp_str = temp_str+"(i) \"Mixed Numbers\"或\"Mix\", "
        temp_str = temp_str+"(ii) \"Organization Numbers\"或\"Org\", "
        temp_str = temp_str+"(iii) \"Another Names\"或\"Virtual Names\"或\"AN\"或\"VN\"."
        ...
        check_valid_page_org_num_frame_multiple_line_1.grid(row = 1, column = 0,
                                                            padx = 2, pady = 2,
                                                            sticky = tk.W)
        temp_str = "(1) 在*.csv文件的第一行，标题行中，有一个必须的格子\"Organization Numbers\"或\"Org\"，以及5个可选的格子(无论大写还是小写字母): "
        temp_str = temp_str+"(i) \"Created Year\"或\"Cre Y\", "
        temp_str = temp_str+"(ii) \"Created Month\"或\"Cre M\", "
        temp_str = temp_str+"(iii) \"Created Day\"或\"Cre D\", "
        temp_str = temp_str+"(iv) \"Created Hour\"或\"Cre H\", "
        temp_str = temp_str+"(v) \"Created Minute\"或\"Cre Min\"."
        ...
        check_valid_page_mani_num_frame_multiple_line_1.grid(row = 1, column = 0,
                                                             padx = 2, pady = 2,
                                                             sticky = tk.W)
        temp_str = "(1) 在*.csv文件的第一行，标题行中，有两个必须的格子(无论大写还是小写字母): "
        temp_str = temp_str+"(i) \"Manipulation Numbers\"或\"Mani\", "
        temp_str = temp_str+"(ii) \"Organization Numbers\"或\"Org\"."
            ...
            if len(main_str_0) == 21:
                ...
                if temp_bool_0:
                    ...
                    if (temp_len_0 < 1) & (temp_len_1 < 1):
                        ...
                        if temp_bool_1:
                            self.check_valid_mix_num_single_state["text"] = "合格 (空白名字)。"
                        else:
                            temp_bool_2 = self.num_mix_valid(main_str_0, None, None, 
                                                             temp_num_2, temp_num_3, temp_num_4, 
                                                             temp_num_5, temp_num_6)
                            if temp_bool_2:
                                self.check_valid_mix_num_single_state["text"] = "合格 (非空白名字)。"
                            else:
                                self.check_valid_mix_num_single_state["text"] = "不合格 (非空白名字)。"
                    elif temp_len_1 < 1:
                        temp_bool_1 = self.num_mix_valid(main_str_0, temp_str_0, temp_str_1, 
                                                         temp_num_2, temp_num_3, temp_num_4, 
                                                         temp_num_5, temp_num_6)
                        if temp_bool_1:
                            self.check_valid_mix_num_single_state["text"] = "合格 (空白姓)。"
                        else:
                            temp_bool_2 = self.num_mix_valid(main_str_0, temp_str_0, None, 
                                                             temp_num_2, temp_num_3, temp_num_4, 
                                                             temp_num_5, temp_num_6)
                            if temp_bool_2:
                                self.check_valid_mix_num_single_state["text"] = "合格。"
                            else:
                                self.check_valid_mix_num_single_state["text"] = "不合格。"
                    else:
                        if temp_len_0 < 1:
                            temp_str_0 = None
                        temp_bool_1 = self.num_mix_valid(main_str_0, temp_str_0, temp_str_1, 
                ...
                if temp_bool_0:
                    ...
                    if temp_len_1 > 0:
                        ...
                        if (temp_len_1 > 0) & (temp_len_1 <= 1000):
                            ...
                            temp_str_0 = temp_str_0+'"'+"索引号"+'"'+","
                            temp_str_0 = temp_str_0+'"'+"合格"+'"'+","
                            temp_str_0 = temp_str_0+'"'+"目标: 混合编号（mixed number）"+'"'+","
                            temp_str_0 = temp_str_0+'"'+"名"+'"'+","
                            temp_str_0 = temp_str_0+'"'+"姓"+'"'+","
                            temp_str_0 = temp_str_0+'"'+"发行年"+'"'+","
                            temp_str_0 = temp_str_0+'"'+"发行月"+'"'+","
                            temp_str_0 = temp_str_0+'"'+"发行日"+'"'+","
                            temp_str_0 = temp_str_0+'"'+"发行时"+'"'+","
                            temp_str_0 = temp_str_0+'"'+"发行分"+'"'
                            with open(temp_path_0, "w", encoding = "utf-8") as save_file:
                                save_file.write(temp_str_0)
                                save_file.close()
                            for n in range(temp_len_1):
                            ...
                            showerror(title = "错误", 
                                      message = "行数（除了第一行，标题行）必须在1-1000.") 
                    else:
                        self.check_valid_mix_num_multiple_state["text"] = "Fail."
                        showerror(title = "错误", 
                                  message = "行数（除了第一行，标题行）必须在1-1000.") 
            ...
            if len(main_str_0) == 14:
                ...
                if temp_bool_0:
                    if len(temp_str_2) < 1:
                        ...
                        if temp_bool_1:
                            self.check_valid_mem_num_single_state["text"] = "合格 (空白其他名字)。"
                        else:
                            temp_bool_2 = self.num_member_valid(main_str_0, temp_str_0, 
                                                                None, temp_str_1)
                            if temp_bool_2:
                                self.check_valid_mem_num_single_state["text"] = "合格 (非空白其他名字)。"
                            else:
                                self.check_valid_mem_num_single_state["text"] = "不合格。"
                    else:
                        temp_bool_1 = self.num_member_valid(main_str_0, temp_str_0, 
                                                            temp_str_2, temp_str_1)
                        if temp_bool_1:
                            self.check_valid_mem_num_single_state["text"] = "合格。"
                        else:
                            self.check_valid_mem_num_single_state["text"] = "不合格。"
                else:
                    self.check_valid_mem_num_single_state["text"] = "不合格。"
                ...
                if temp_bool_0:
                    ...
                    if temp_len_1 > 0:
                        ...
                        if (temp_len_1 > 0) & (temp_len_1 <= 1000):
                            ...
                            temp_str_0 = temp_str_0+'"'+"索引号"+'"'+","
                            temp_str_0 = temp_str_0+'"'+"合格"+'"'+","
                            temp_str_0 = temp_str_0+'"'+"目标: 成员编号"+'"'+","
                            temp_str_0 = temp_str_0+'"'+"混合编号"+'"'+","
                            temp_str_0 = temp_str_0+'"'+"发行方的组织编号"+'"'+","
                            temp_str_0 = temp_str_0+'"'+"其他名字 / 虚拟名字"+'"'
                            with open(temp_path_0, "w", encoding = "utf-8") as save_file:
                                save_file.write(temp_str_0)
                                save_file.close()
        ...
        if not temp_bool_0:
            ...
            if len(main_str_0) == 14:
                ...
                if temp_bool_0:
                    ...
                    if temp_bool_1:
                        self.check_valid_org_num_single_state["text"] = "合格。"
                    else:
                        self.check_valid_org_num_single_state["text"] = "不合格。"
                else:
                    self.check_valid_org_num_single_state["text"] = "不合格。"
            else:
                self.check_valid_org_num_single_state["text"] = "不合格。"
                ...
                if temp_bool_0:
                    ...
                    if temp_len_1 > 0:
                        ...
                        if (temp_len_1 > 0) & (temp_len_1 <= 1000):
                            ...
                            temp_str_0 = temp_str_0+'"'+"索引号"+'"'+","
                            temp_str_0 = temp_str_0+'"'+"合格"+'"'+","
                            temp_str_0 = temp_str_0+'"'+"目标: 组织编号"+'"'+","
                            temp_str_0 = temp_str_0+'"'+"创建年"+'"'+","
                            temp_str_0 = temp_str_0+'"'+"创建月"+'"'+","
                            temp_str_0 = temp_str_0+'"'+"创建日"+'"'+","
                            temp_str_0 = temp_str_0+'"'+"创建时"+'"'+","
                            temp_str_0 = temp_str_0+'"'+"创建分"+'"'
                            with open(temp_path_0, "w", encoding = "utf-8") as save_file:
                                save_file.write(temp_str_0)
                                save_file.close()
                            for n in range(temp_len_1):
        ...
        if not temp_bool_0:
            ...
            if len(main_str_0) == 7:     
                if len(temp_str_0) == 14:
                   ...
                   if temp_bool_1:
                       self.check_valid_mani_num_single_state["text"] = "合格。"
                   else:
                       self.check_valid_mani_num_single_state["text"] = "不合格。"
                else:
                    self.check_valid_mani_num_single_state["text"] = "不合格。"
            else:
                self.check_valid_mani_num_single_state["text"] = "不合格。"
                ...
                if temp_bool_0:
                    ...
                    if temp_len_1 > 0:
                        ...
                        if (temp_len_1 > 0) & (temp_len_1 <= 1000):
                            ...
                            temp_str_0 = temp_str_0+'"'+"索引号"+'"'+","
                            temp_str_0 = temp_str_0+'"'+"合格"+'"'+","
                            temp_str_0 = temp_str_0+'"'+"目标: 操作编号"+'"'+","
                            temp_str_0 = temp_str_0+'"'+"组织编号"+'"'
                            with open(temp_path_0, "w", encoding = "utf-8") as save_file:
                                save_file.write(temp_str_0)
                                save_file.close()
                            for n in range(temp_len_1):
    ...
    def output_csv_page(self):
        ...
        output_csv_page_mem_frame_line_0.grid(row = 0, column = 0,
                                              padx = 2, pady = 2, 
                                              sticky = tk.W)
        temp_str_0 = "1. 扫描数据库的*.iden和*.txt, '"
        temp_str_0 = temp_str_0+self.basic_parameter[4][0]+"/"
        temp_str_0 = temp_str_0+"', 然后在数据库的槽中生成拷贝文件*.iden."
        ...
        output_csv_page_mem_frame_line_1.grid(row = 2, column = 0,
                                              padx = 2, pady = 2, 
                                              sticky = tk.W)
        temp_str_0 = "2. 生成*.csv至'"
        temp_str_0 = temp_str_0+self.basic_parameter[3][0]+"/"
        temp_str_0 = temp_str_0+"' 来自数据库的槽'"
        temp_str_0 = temp_str_0+self.basic_parameter[4][0]+"/"
        temp_str_0 = temp_str_0+"'."
        ...
        if not self.cur_progress_bool:
            ...
            if temp_str == "mem":
                ...
                if not self.cur_org_file_name is None:
                    self.output_csv_page_org_mani_list_state["text"] = "文件来自'assets/"+self.cur_org_file_name+"'."
                    self.output_csv_page_org_mani_list_bool = True
                else:
                    self.output_csv_page_org_mani_list_state["text"] = "没有*.dat文件。"
                    self.output_csv_page_org_mani_list_bool = False
                if not self.basic_org_list is None:
                    if not self.basic_org_list[1] is None:
                        if self.basic_org_list[0][0][0:3].upper() == self.cur_org[0][0][0:3].upper():
                            self.output_csv_page_org_org_list_state["text"] = "从'assets/OrganizationList.dat'中预读文件."
                            self.output_csv_page_org_org_list_bool = True
                        else:
                            self.output_csv_page_org_org_list_state["text"] = "从'assets/OrganizationList.dat'中无法输出."
                            self.output_csv_page_org_org_list_bool = False
                    else:
                        self.output_csv_page_org_org_list_state["text"] = "无法读取'assets/OrganizationList.dat'."
                        self.output_csv_page_org_org_list_bool = False
                else:
                    self.output_csv_page_org_org_list_state["text"] = "没有*.dat文件是'assets/OrganizationList.dat'."
                    self.output_csv_page_org_org_list_bool = False
        else:
            self.output_csv_page_mem_radio.select()
                    ...
                    for e in temp_file_list:
                        if self.cur_progress_bool:
                            ...
                            if len(temp_file_type_if_iden_list) > 20000:
                                temp_bool_0 = False
                                showerror(title = "错误", 
                                          message = "超过了20 000个*.txt或*.iden文件。")
                                break
                        else:
                            break
                    temp_bool_2 = False
                    if self.cur_progress_bool:
                        if temp_bool_0:
                            ...
                            if temp_len_0 > 0:
                                ...
                                temp_str_0 = temp_str_0+'"'+"索引号"+'"'+","
                                temp_str_0 = temp_str_0+'"'+"合格"+'"'+","
                                temp_str_0 = temp_str_0+'"'+"如果与先前的重复"+'"'+","
                                temp_str_0 = temp_str_0+'"'+"文件名字"+'"'+","
                                temp_str_0 = temp_str_0+'"'+"混合编号 (mixed number)"+'"'+","
                                temp_str_0 = temp_str_0+'"'+"成员编号 (member number)"+'"'+","
                                temp_str_0 = temp_str_0+'"'+"名"+'"'+","
                                temp_str_0 = temp_str_0+'"'+"中间名"+'"'+","
                                ...
                                for n in range(temp_len_0):
                                    ...
                                    temp_str_1 = temp_str_1+"已扫描，来自数据库'"
                                    temp_str_1 = temp_str_1+self.basic_parameter[4][0]+"/'."
                                    self.output_csv_page_mem_state["text"] = temp_str_1
                                    self.update()
                            else:
                                showerror(title = "错误", 
                                          message = "没有*.txt或*.iden文件。")
                    if self.cur_progress_bool:
                        self.cur_progress_bool = False
                        self.output_csv_page_cur_process_vari = ""
                        if temp_bool_2:
                            self.output_csv_page_mem_scan_state["text"] = "完成。"
                            self.output_csv_page_mem_state["text"] = "'"+temp_path_0+"'已输出，含有"+str(len(self.output_csv_mem_name_copy_list))+"个合格结果。"
                        else:
                            self.output_csv_mem_name_copy_from_list = None
                            self.output_csv_mem_name_copy_list = None
                            self.output_csv_mem_content_list = None
                            self.output_csv_page_mem_state["text"] = "没有合格结果。"
                    else:
                        if temp_bool_2:
                            self.output_csv_page_mem_state["text"] = "'"+temp_path_0+"'已输出。"
                        else:
                            self.output_csv_mem_name_copy_from_list = None
                            self.output_csv_mem_name_copy_list = None
                            self.output_csv_mem_content_list = None      
    ...
    def output_csv_fun_copy(self, in_num):
        if not self.cur_progress_bool:
            ...
            if not temp_bool_0:
                ...
                temp_str_0 = "槽-"+str(in_num+1)
                temp_str_1 = "您确定要开始拷贝至"
                temp_str_1 = temp_str_1+temp_str_0+"?"
                answer = askyesno(title="拷贝",
                                  message=temp_str_1)
                if answer:
                    if self.output_csv_mem_content_list is None:
                        ...
                        temp_str_1 = temp_str_1+'"'+"索引号"+'"'+","
                        temp_str_1 = temp_str_1+'"'+"如果成功拷贝"+'"'+","
                        temp_str_1 = temp_str_1+'"'+"在数据库中的原文件名字"+'"'+","
                        temp_str_1 = temp_str_1+'"'+"文件名字在"+temp_str_0+'"'
                        with open(temp_path_0, "w", encoding = "utf-8") as save_file:
                            save_file.write(temp_str_1)
                            save_file.close()
                        ...
                        for n in range(temp_len_0):
                            if self.cur_progress_bool:
                                ...
                                temp_str_2 = temp_str_2+"已拷贝至"+temp_str_0
                        ...
                        if self.cur_progress_bool:
                            self.cur_progress_bool = False
                            self.output_csv_page_cur_process_vari = ""
                            self.output_csv_page_mem_copy_state["text"] = "完成。"
                            self.output_csv_page_mem_state["text"] = "'"+temp_path_0+"'已输出。"
                        else:
                            self.output_csv_page_cur_process_vari = ""
                ...
                if answer:
                    ...
                    if temp_len_0 < 1:
                            ...
                            if self.cur_progress_bool:
                                ...
                                if temp_bool_1:
                                    ...
                                    if temp_num_0 == 0:
                                        ...
                                        temp_str_3 = temp_str_3+'"'+"索引号"+'"'+","
                                        temp_str_3 = temp_str_3+'"'+"混合编号(mixed number)"+'"'+","
                                        temp_str_3 = temp_str_3+'"'+"成员编号(member number)"+'"'+","
                                        temp_str_3 = temp_str_3+'"'+"发行日期"+'"'+","
                                        temp_str_3 = temp_str_3+'"'+"发行时间"+'"'+","
                                        temp_str_3 = temp_str_3+'"'+"英文名"+'"'+","
                                        temp_str_3 = temp_str_3+'"'+"英文中间名"+'"'+","
                                        temp_str_3 = temp_str_3+'"'+"英文姓"+'"'+","
                                        temp_str_3 = temp_str_3+'"'+"其他名字 / 虚拟名字 的类型"+'"'+","
                                        temp_str_3 = temp_str_3+'"'+"其他名字 / 虚拟名字"+'"'+","
                                        temp_str_3 = temp_str_3+'"'+"其他名字 / 虚拟名字 的附加 (@或#)"+'"'+","
                                        temp_str_3 = temp_str_3+'"'+"发行方的操作编号(manipulation number)"+'"'+","
                                        temp_str_3 = temp_str_3+'"'+"发行方的组织编号(organization number)"+'"'+","
                                        temp_str_3 = temp_str_3+'"'+"发行方的组织名字"+'"'+","
                                        temp_str_3 = temp_str_3+'"'+"组织的创建日期"+'"'+","
                                        temp_str_3 = temp_str_3+'"'+"组织的创建时间"+'"'
                                    ...
                                    temp_str_4 = temp_str_4+"\n总文件数量，在'"+self.basic_parameter[4][1][in_num]+"/': "
                                    temp_str_4 = temp_str_4+str(temp_success_num+temp_fail_num)
                                    temp_str_4 = temp_str_4+"\n总表格数量，在'"+self.basic_parameter[3][1][in_num]+"/': "
                                    ...
                                    temp_str_4 = temp_str_4+"\n总文件数量，在'"+self.basic_parameter[4][1][in_num]+"/': "
                                    temp_str_4 = temp_str_4+str(temp_success_num+temp_fail_num)
                                    temp_str_4 = temp_str_4+"\n总编号数量，在'"+self.basic_parameter[3][1][in_num]+"/': "
                                    temp_str_4 = temp_str_4+str(temp_num_1)
                                    with open(temp_path_1, "w", encoding = "utf-8") as save_file:
                                        save_file.write(temp_str_4)
                                        save_file.close()
                                ...
                                temp_str_3 = temp_str_3+", 其中"+str(temp_success_num)
                                temp_str_3 = temp_str_3+"已输出至表，在'"+self.basic_parameter[3][1][in_num]+"/'."
                        ...
                        if self.cur_progress_bool:
                            self.cur_progress_bool = False
                            self.output_csv_page_cur_process_vari = ""
                            self.output_csv_page_mem_output_state["text"] = "完成。"
                            self.output_csv_page_mem_state["text"] = "'"+temp_path_1+"'已输出。"
                        else:
                            self.output_csv_page_cur_process_vari = ""
        ...
        if self.cur_progress_bool:
            if in_str == self.output_csv_page_cur_process_vari:
                ...
                if answer:  
                    ...
                    self.output_csv_page_cur_process_vari = ""
                    if in_str == "scan":
                        self.output_csv_page_mem_scan_state["text"] = "停止。"
                    elif in_str == "copy":
                        self.output_csv_page_mem_copy_state["text"] = "停止。"
                    elif in_str == "output":
                        self.output_csv_page_mem_output_state["text"] = "停止。"
    ...
    def output_csv_fun_mani(self):
        if self.output_csv_page_org_mani_list_bool: 
            ...
            if not temp_bool_0:
                ...
                temp_str_0 = chr(65279)
                temp_str_0 = temp_str_0+'"'+"索引号"+'"'+","
                temp_str_0 = temp_str_0+'"'+"如果是许可的"+'"'+","
                temp_str_0 = temp_str_0+'"'+"操作编号(manipulation number)"+'"'+","
                temp_str_0 = temp_str_0+'"'+"当前组织编号(organization number)"+'"'+","
                temp_str_0 = temp_str_0+'"'+"当前组织名称"+'"'+","
                temp_str_0 = temp_str_0+'"'+"如果是创建者"+'"'+","
                temp_str_0 = temp_str_0+'"'+"创建日期"+'"'+","
                temp_str_0 = temp_str_0+'"'+"创建时间"+'"'+","
                temp_str_0 = temp_str_0+'"'+"如果是管理员"+'"'+","
                temp_str_0 = temp_str_0+'"'+"成员编号（member number）"+'"'+","
                temp_str_0 = temp_str_0+'"'+"名字类型"+'"'+","
                temp_str_0 = temp_str_0+'"'+"英文名"+'"'+","
                temp_str_0 = temp_str_0+'"'+"英文中间名"+'"'+","
                temp_str_0 = temp_str_0+'"'+"英文姓"+'"'+","
                temp_str_0 = temp_str_0+'"'+"其他名字 / 虚拟名字 的类型"+'"'+","
                temp_str_0 = temp_str_0+'"'+"其他名字 / 虚拟名字"+'"'+","
                temp_str_0 = temp_str_0+'"'+"其他名字 / 虚拟名字 的附加 (@或#)"+'"'+","
                temp_str_0 = temp_str_0+'"'+"发行方的组织编号"+'"'+","
                temp_str_0 = temp_str_0+'"'+"发行方的创建日期"+'"'+","
                temp_str_0 = temp_str_0+'"'+"发行方的创建时间"+'"'
                ...
                for n in range(len(self.cur_org[1])):
                    ...
                    if self.cur_org[1][n][3] == "en":
                        ...
                        temp_str_1 = temp_str_1+"其他名字 / 虚拟名字"
                        temp_str_1 = temp_str_1+","
                        temp_str_1 = temp_str_1+","
                        temp_str_1 = temp_str_1+","
                        temp_str_1 = temp_str_1+","
                    ...
                    with open(temp_path_0, "a", encoding = "utf-8") as save_file:
                        save_file.write(temp_str_1)
                        save_file.close()
                showinfo(title = "*.csv文件已输出", 
                         message = "*.csv文件'"+temp_path_0+"'已输出。")
    ...
    def output_csv_fun_org(self):
        if self.output_csv_page_org_org_list_bool: 
            ...
            if not temp_bool_0:
                ...
                temp_str_0 = chr(65279)
                temp_str_0 = temp_str_0+'"'+"索引号"+'"'+","
                temp_str_0 = temp_str_0+'"'+"组织编号(organization number)"+'"'+","
                temp_str_0 = temp_str_0+'"'+"组织名称"+'"'+","
                temp_str_0 = temp_str_0+'"'+"创建日期"+'"'+","
                temp_str_0 = temp_str_0+'"'+"创建时间"+'"'+","
                temp_str_0 = temp_str_0+'"'+"创建者的操作编号(manipulation number)"+'"'
    ...
    def setting_page(self):    
        ...
        ttk.Label(setting_page_new_org_note_frame, 
                  ...
                  font = self.fonsize_s).grid(row = 0, column = 0,
                                              sticky = tk.W)
        temp_str_0 = "    (1) 英文名的第一个字符是来自26个英文大写字母或者数字0-9。"
        ttk.Label(setting_page_new_org_note_frame, 
                  text = temp_str_0,    
                  wraplength = 1000,                       
                  font = self.fonsize_s).grid(row = 1, column = 0,
                                              sticky = tk.W)
        temp_str_0 = "    (2) 英文名的其它字符来自: "
        temp_str_0 = temp_str_0+"(i) 26个英文大写字母, "
        temp_str_0 = temp_str_0+"(ii) 26个英文小写字母, "
        temp_str_0 = temp_str_0+"(iii) 数字0-9, "
        temp_str_0 = temp_str_0+"(iv) 空格\" \", "
        temp_str_0 = temp_str_0+"(v) 横线\"-\", "
        temp_str_0 = temp_str_0+"(vi) 和\"&\", "
        temp_str_0 = temp_str_0+"(vii) 或\"/\", "
        temp_str_0 = temp_str_0+"(iix) 单引号\"'\"和\"‘\"和\"’\", "
        temp_str_0 = temp_str_0+"(ix) 点\".\", "
        temp_str_0 = temp_str_0+"(x) 冒号\":\", "
        temp_str_0 = temp_str_0+"(xi) 小括号\"(\"和\")\"."
        ttk.Label(setting_page_new_org_note_frame, 
                  text = temp_str_0,     
                  wraplength = 1000,                       
                  font = self.fonsize_s).grid(row = 2, column = 0,
                                              sticky = tk.W)
        temp_str_0 = "    (3) 英文名的最大长度是"+str(self.org_name_max_len)+"."
        ttk.Label(setting_page_new_org_note_frame, 
                  text = temp_str_0,  
                  wraplength = 1000,                        
                  font = self.fonsize_s).grid(row = 3, column = 0,
                                              sticky = tk.W)
        temp_str_0 = "    (4) 编号的初始3个数位必须来自26个英文字母，无论是大写还是小写。"
        ttk.Label(setting_page_new_org_note_frame, 
                  text = temp_str_0,  
                  wraplength = 1000,                        
        ...
        if not self.sel_org is None:
            if temp_str_0 in self.sel_org[2]:
                ...
                with open("assets/NumGene.dat", "w", encoding = "utf-8") as save_file:
                    save_file.write(temp_str_1)
                    save_file.close()
                showinfo(title = "操作编号已设置", 
                         message = "组织编号: "+self.cur_org[0][0]+"\n操作编号:"+self.cur_mani_num+"\n已设置。")
                self.setting_mani_num_entry.delete(0, "end")
                self.login_state = True
            else:
                showerror(title = "错误", 
                          message = "该操作编号不在该 组织.dat 文件中.") 
        else:
            showerror(title = "错误", 
                      message = "无法读取该 组织.dat 文件。") 
                                ...
                                if not out_str_list_1 is None:  
                                    ...
                                    if temp_bool_0:
                                        ...
                                        with open(temp_str_2, "w", encoding = "utf-8") as save_file:
                                            ...
                                            save_file.close()
                                            showinfo(title = "输出*.dat文件", 
                                                     message = "文件'"+temp_str_2+"'已输出。\n管理员的操作编号是'"+self.setting_new_org_str_1+"'。")
                                        self.setting_gene_org_frame_0.grid(row = 1, column = 0,
                                                                           padx = 5, pady = 5, 
                                                                           sticky = tk.NW)
                                        self.setting_gene_org_frame_1.grid_forget()            
    ...
    def setting_fun_my_org_switch_page(self):
        if not self.cur_org is None:
            ...
            if temp_str == "info":
                ...
                self.setting_my_org_mani_listbox_select["text"] = "当前选择: -"
    ...
    def setting_fun_my_org_confirm_org_name(self):
        if not self.cur_org is None:
            ...
            if temp_bool:
                ...
                temp_str = "(1) 英文名的第一个字符是来自26个英文大写字母或者数字0-9。"
                temp_str = temp_str+"\n\n"
                temp_str = temp_str+"(2) 其它字符来自: "
                temp_str = temp_str+"(i) 26个英文大写字母, "
                temp_str = temp_str+"(ii) 26个英文小写字母, "
                temp_str = temp_str+"(iii) 数字0-9, "
                temp_str = temp_str+"(iv) 空格\" \", "
                temp_str = temp_str+"(v) 横线\"-\", "
                temp_str = temp_str+"(vi) 和\"&\", "
                temp_str = temp_str+"(vii) 或\"/\", "
                temp_str = temp_str+"(iix) 单引号\"'\"和\"‘\"和\"’\", "
                temp_str = temp_str+"(ix) 点\".\", "
                temp_str = temp_str+"(x) 冒号\":\", "
                temp_str = temp_str+"(xi) parenthesis \"(\" and \")\"."
                temp_str = temp_str+"\n\n"
                temp_str = temp_str+"(3) 最大长度是"+str(self.org_name_max_len)+"。"
                showerror(title = "错误", 
                          message = temp_str) 
    ...
    def setting_fun_my_org_confirm_city(self):
        if not self.cur_org is None:
            ...
            if temp_bool:
                ...
                temp_str = temp_str+"(ix) 点\".\", "
                temp_str = temp_str+"(x) 冒号\":\", "
                temp_str = temp_str+"(xi) 小括号\"(\"和\")\"."
                temp_str = temp_str+"\n\n"
                temp_str = temp_str+"(3) 长度没有限制，甚至可以长度为0。"
                showerror(title = "错误", 
                          message = temp_str) 
        ...
        if not self.cur_org is None:
            ...
            for n in range(temp_len):
                ...
                if not temp_tuple is None:
                    ...
                    if temp_tuple[1]:
                        temp_str = temp_str+"许可的"
                    else:
                        temp_str = temp_str+"非许可"
                    temp_str = temp_str+" - "
                    ...
                    temp_str = ""
                    temp_str = str(n+1)+"/"+str(temp_len)+" - <空白>"
                ...
                if not temp_tuple is None:
                    if temp_tuple[0] == self.cur_org[0][4]:
                        ...
                        showerror(title = "错误", 
                                  message = "上一次编辑者不能被移除。")
                if temp_bool:
                    answer = askyesno(title="移除",
                                      message="您确定要移除项目"+str(1+self.setting_my_org_mani_cur_sel)+"?")
                    if answer:  
                        self.setting_my_org_mani_list_listbox.select_clear(self.setting_my_org_mani_cur_sel)
        ...
        if not self.cur_org is None:
            if self.setting_my_org_mani_cur_sel >= 0:
                ...
                if temp_bool:
                    ...
                    self.setting_my_org_mani_listbox_select["text"] = temp_str+"已修改."
                    self.setting_my_org_mani_list_frame_line_0_Right.grid_forget()
                    self.setting_my_org_mani_org_frame.grid_forget()
                    self.setting_my_org_mani_cur_sel = -1
                else:
    ...
    def setting_fun_my_org_output(self):
        if not self.cur_org is None:
            ...
            out_str_0 = self.forming_str_text_org(self.cur_org[0][0], self.cur_org[0][2], temp_str_1, 
                                                  self.setting_my_org_admin_num_vari, self.cur_org[0][5], self.cur_mani_num, 
            ...
            if not out_str_0 is None:
                ...
                with open(temp_str_2, "w", encoding = "utf-8") as save_file:
                    ...
                    save_file.close()
                    showinfo(title = "输出*.dat文件", 
                             message = "文件'"+temp_str_2+"'已输出。\n管理员的操作编号是'"+self.setting_my_org_admin_num_vari+"'。")
            else:
                showerror(title = "错误", 
                          message = "内容格式错误，文件无法输出。")
    ...
    def setting_fun_org_list_read(self):
        ...
        self.setting_org_list_cur_sel = -1
        self.setting_org_list_listbox_select["text"] = "当前选择: -"
    ...
    def setting_fun_org_list_listbox_remove(self):
        if not self.cur_org_list is None:
            if self.setting_org_list_cur_sel >= 0:              
                answer = askyesno(title="移除",
                                  message="您确定要移除项目"+str(1+self.setting_org_list_cur_sel)+"?")
                if answer:  
                    self.setting_org_list_listbox.select_clear(self.setting_org_list_cur_sel)
        ...
        if not self.cur_org_list is None:
            if self.setting_org_list_cur_sel >= 0:
                ...
                if temp_bool:                    
                    ...
                    temp_str = temp_str+"/"
                    temp_str = temp_str+str(len(self.cur_org[1]))
                    self.setting_org_list_listbox_select["text"] = temp_str+"已修改。"
                    self.setting_org_list_right_frame.grid_forget()  
                    self.setting_org_list_cur_sel = -1
                else:
                    showerror(title = "错误", 
    ...
    def other_page(self):
        ...
        self.other_page_scan_validity_radio = tk.Radiobutton(other_page_radio_line_0, 
                                                             ...
                                                             text = "扫描一个*.iden或*.txt文件来检查是否为合格的成员信息", 
                                                             value = "valid", 
                                                             font = self.fonsize_s)
        ...
        temp_str_0 = temp_str_0+"\n"
        temp_str_0 = temp_str_0+"    (5) For each case of first 2 letters of English given name, first 2 letters of English family name, there are possibly 1.405669E+16 different mixed numbers in the same minute (because the large number in every minute, this algorithm can support separably distributed structure without Internet);"
        temp_str_0 = temp_str_0+"\n"
        ...
        self.other_rule_pad.insert("insert", self.forming_rule_str_zhs())
        ...
        temp_str_0 = temp_str_0+"\n\n"
        temp_str_0 = temp_str_0+"June 9, 2022"
        temp_str_0 = temp_str_0+"\n\n\n\n"
        ...
        temp_str_0 = temp_str_0+"On February 24, 2022, when Russia invaded Ukraine, I noticed that 'the wars are not far from us' and, I decided to accept the modified algorithm, which is the current algorithm. "
        temp_str_0 = temp_str_0+"\n\n"
        temp_str_0 = temp_str_0+"5. In March 2022, the Chinese Communist Party (CCP) interrupted the Ukraine rescue of the NFSC, "
        ...
        temp_str_0 = temp_str_0+"\n\n"
        temp_str_0 = temp_str_0+"6. Since the end of March 2022, Shanghai, the largest city in Mainland China, was locked down by the CCP, "
        temp_str_0 = temp_str_0+"and many cities in Mainland China followed. The lockdowns had caused large number of deaths and other humanitarian crises."
        temp_str_0 = temp_str_0+"\n\n"
        ...
        temp_str_0 = temp_str_0+"and to bring freedom, democracy, rule-of-law to Chinese people is one of my goals, and this number generator can assist Chinese people to construct the democratic authority. "
        temp_str_0 = temp_str_0+"\n"
        temp_str_0 = temp_str_0+"As I am a scientist, even though I don't know how to end the humanitarian crises in Mainland China, Ukraine, Russia, and even possibly to avoid humanitarian crises in Taiwan if CCP will try to conquer Taiwan, "
        temp_str_0 = temp_str_0+"my hope is still that I can apply my knowledge to help more people, and this number generator can assist people in dangerous area to construct their own database regionally."
        temp_str_0 = temp_str_0+"\n\n\n\n"
        ...
        if file:
            ...
            with open(file_path, "r", encoding = "utf-8") as open_file:
                ...
                if not self.cur_scan_file_info is None:
                    ...
                    temp_str_0 = temp_str_0+"\n\n\n"
                    temp_str_0 = temp_str_0+"成员的 其他名字 / 虚拟名字: "
                    temp_str_0 = temp_str_0+"\n\n"
                    temp_str_0 = temp_str_0+"类型, "
                    temp_str_0 = temp_str_0+"\n"
                    temp_str_0 = temp_str_0+self.cur_scan_file_info[2][0]
                    temp_str_0 = temp_str_0+"\n\n"
                    temp_str_0 = temp_str_0+"名字, "
                    temp_str_0 = temp_str_0+"\n"
                    ...
                    temp_str_0 = temp_str_0+"\n\n\n"
                    temp_str_0 = temp_str_0+"发行方: "
                    temp_str_0 = temp_str_0+"\n\n"
                    temp_str_0 = temp_str_0+"操作编号（manipulation number，7个数位）, "
                    temp_str_0 = temp_str_0+"\n"
                    temp_str_0 = temp_str_0+self.cur_scan_file_info[4][3]
                    temp_str_0 = temp_str_0+"\n\n"
                    temp_str_0 = temp_str_0+"组织编号（organization number，14个数位）, "
                    temp_str_0 = temp_str_0+"\n"
                    ...
                    if not self.basic_org_list is None:
                        if not self.basic_org_list[1] is None:
                            ...
                            if temp_num_1 >= 0:
                                ...
                                temp_str_0 = temp_str_0+"没有结果"
                        else:
                            temp_str_0 = temp_str_0+"因为文件'assets/OrganizationList.dat'已损坏，无法用组织编号进行搜索。"
                    else:
                        temp_str_0 = temp_str_0+"因为文件'assets/OrganizationList.dat'不存在，无法用组织编号进行搜索。"
                    temp_str_0 = temp_str_0+"\n\n"
                    temp_str_0 = temp_str_0+"该组织的创建时间"
                    ...
                    temp_str_0 = file_path
                    temp_str_0 = temp_str_0+"\n\n"
                    temp_str_0 = temp_str_0+"该文件不是合格的。"
                    temp_str_0 = temp_str_0+"\n\n\n\n"
                    self.other_scan_valid_result_pad.insert("insert", temp_str_0)
                    self.other_scan_valid_result_pad["state"] = "disabled"
                    self.other_scan_valid_save_iden_button["state"] = "disabled"
    ...
    def forming_rule_str_zhs(self):
        ...
        temp_str_0 = temp_str_0+"\n"
        temp_str_0 = temp_str_0+"    (5) 对于英文名的前2个字母、英文姓的前2个字母的每一种情况，在同一分钟内有1.405669E+16个不同的混合编号（因为每一分钟的巨大数量，该算法能够支持没有互联网的可分离型分布式结构）；"
        temp_str_0 = temp_str_0+"\n"
    ...
    def forming_rule_str_zht(self):
        ...
        temp_str_0 = temp_str_0+"\n"
        temp_str_0 = temp_str_0+"    (5) 對於英文名的前2個字母、英文姓的前2個字母中的每一種情況，在同一分鐘內有1.405669E+16個不同的混合編號（因為每一分鐘的巨大數量，該算法能夠支持沒有互聯網的可分離型分布式結構）；"
        temp_str_0 = temp_str_0+"\n"
